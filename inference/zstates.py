...
def create(scaledparams, x, y, cmax, nvar, target, prior):
    ...
    posterior   = lml.prob_comps(scaledparams, x, y, zstates + newk, prior)
    ...
    print("Old probsum: {:f}, target: {:f}".format(old_probsum, target))
    selk = prune(newk, prob, old_probsum, target)
    if len(selk) > 0:
        zstates += selk

    print("Working with "+str(len(zstates))+" leading zstates.")
