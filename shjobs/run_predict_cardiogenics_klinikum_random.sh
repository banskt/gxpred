...
for CHROM in `seq 1 2`
do

	ENV='/usr/users/fsimone/myenv'
	PREDICT='/usr/users/fsimone/gxpred/predict_gtex.py'
	MODELPATH='/usr/users/fsimone/datasets/gtex/gxpred_models_klinikum_random'
	# CHROM="1"
	GTPATH="/usr/users/fsimone/datasets/cardiogenics/genotypes"
	GTFILE="CG_${CHROM}.imputed.gz"
	LOGDIR="/usr/users/fsimone/datasets/cardiogenics/gxpred_predictions_klinikum_random"
	OUTDIR="/usr/users/fsimone/datasets/cardiogenics/gxpred_predictions_klinikum_random/pred_chr${CHROM}"
