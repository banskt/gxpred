...
GXPRED='/usr/users/fsimone/gxpred/learn_gtex_specific_gene_test.py'
...
echo bsub -x -q ${QUEUE} -R span[hosts=1] -o /usr/users/fsimone/memtest_genenum_${GENE}_${LOG_SUFFIX}.log -e ${OUTDIR}/section${SECTION}_${LOG_SUFFIX}.err\
								$ENV/bin/python $GXPRED \
								--gen ${GTFOLDER}/${GTFILE} --sample ${GTFOLDER}/${SAMPLEFILE} \
								--gtf ${GTFPATH} --chr ${CHROM} \
								--expr ${EXPR} --out ${OUTDIR} --genenum ${GENE} --method ${METHOD} --zstates ${ZMAX}
